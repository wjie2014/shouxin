...
public class PairAccess {
    ...
    public Map<String,Object> require(String id, Authentication auth, boolean write) {
        ...
        boolean reviewer=!write&&u.roles().stream().anyMatch(r->Set.of("QA_REVIEW_L1","QA_REVIEW_L2","QA_REVIEW_L3").contains(r));
        ...
        if (!admin && !reviewer && !u.id().equals(p.get("author_id")) && !assigned) throw new AccessDeniedException("无权访问该问答对");
