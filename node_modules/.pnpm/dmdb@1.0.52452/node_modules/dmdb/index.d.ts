...
declare namespace dmdb {
    ...
    /** 
     * 绑出参数的返回格式——自动（按位置绑定时为数组，按名称绑定时为对象）
     * @value 4003
     */
    const OUT_FORMAT_AUTO: number;
    /**
     * 数据类型——BLOB
     * @value 2019
     */
    ...
    /**
     * 结果集最大行数限制，超过的截断；0表示无限制
     * 结果集最大行数配置优先级顺序从高到低如下：
     * （1）执行选项 ExecuteOptions.maxRows
     * （2）驱动全局配置 dmdb.maxRows
     * （3）连接串属性/连接属性 localhost:5236?maxRows=0 / ConnectionAttributes.maxRows
     * @default undefined
     */
    let maxRows: number | undefined;
    /**
     * 绑出参数返回格式：数组，对象或自动。
     * 取值范围：OUT_FORMAT_ARRAY、OUT_FORMAT_OBJECT、OUT_FORMAT_AUTO
     * @default OUT_FORMAT_AUTO
     */
    let outBindFormat: number;
    /**
     * 结果返回格式：数组或对象。
    ...
    interface ConnectionAttributes {
         ...
         * @default 0
         */
        pstmtPoolSize?: number;
        /**
         * 预编译语句在缓存池中的有效时间，单位ms；0表示无限制
         * @default 0
         */
        pstmtPoolValidTime?: number;
        /**
